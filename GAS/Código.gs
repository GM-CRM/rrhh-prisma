...
function doPost(e) {
  try {
    ...
    switch (body.action) {
      ...
      case "generar_contrato":      r = generarContrato(body.payload);       break;
      case "obtener_datos_contrato": r = obtenerDatosContrato(body.payload);  break;
      // ── AUTH ──────────────────────────────────────────────
...
/* ============================================================
   GENERACIÓN DE CONTRATOS — Prisma RH
   (integrado: router ya actualizado arriba, funciones abajo)
   ============================================================ */

var ID_PLANTILLA_CONTRATO_OPERATIVO      = "1cT3Jgwqw1GSPBqXIWCseTvA6fNGsWRpdzuJ5EhCdfa4";
var ID_PLANTILLA_CONTRATO_ADMINISTRATIVO = "1AtCDrwi2WyfExAn1vJMt7lP002epCeMP7AEbgQvBFnA";

// ─── PRELLENAR EL FORMULARIO ────────────────────────────────────
function obtenerDatosContrato(payload) {
  var idInterno = (payload.idInterno || "").toString().trim();
  if (!idInterno) return { status: "error", message: "Falta ID INTERNO." };
  ...
  try {
    ...
    var idx = {
      idInterno:   headers.indexOf("ID INTERNO"),
      nombre:      headers.indexOf("NOMBRE DEL TRABAJADOR"),
      puesto:      headers.indexOf("PUESTO"),
      tipoIngreso: headers.indexOf("TIPO DE INGRESO"),
      fechaInicio: headers.indexOf("FECHA DE INICIO DEL PRIMER CONTRATO"),
      fechaFinOperativo: headers.indexOf("FECHA DE VENCIMIENTO DEL TERCER CONTRATO"),
      fechaFinAdmin:     headers.indexOf("FECHA DE VENCIMIENTO DEL SEXTO CONTRATO")
    };
    if (idx.idInterno === -1) idx.idInterno = 50; // AY, mismo fallback que el resto del proyecto

    for (var i = 1; i < data.length; i++) {
      if ((data[i][idx.idInterno] || "").toString().trim() === idInterno) {
        var row = data[i];
        var tipoIngreso = (row[idx.tipoIngreso] || "").toString().trim();
        var esOperativo = /operativ/i.test(tipoIngreso);
        var fechaInicioRaw = row[idx.fechaInicio];
        var fechaFinRaw     = esOperativo ? row[idx.fechaFinOperativo] : row[idx.fechaFinAdmin];
        ...
        return {
          ...
          datos: {
            ...
            puesto:      (row[idx.puesto] || "").toString().trim(),
            tipoIngreso: tipoIngreso,
            fechaInicio: _serialAFechaISO(fechaInicioRaw),
            fechaFin:    _serialAFechaISO(fechaFinRaw)
          }
        };
      }
    }
...
function _serialAFechaISO(valor) {
  if (!valor) return "";
  var fecha;
  if (valor instanceof Date) {
    ...
    var epocaSheets = new Date(Date.UTC(1899, 11, 30));
    fecha = new Date(epocaSheets.getTime() + valor * 86400000);
  } else {
    return "";
  }
  var y = fecha.getUTCFullYear();
  var m = (fecha.getUTCMonth() + 1).toString().padStart(2, "0");
  var d = fecha.getUTCDate().toString().padStart(2, "0");
  return y + "-" + m + "-" + d;
}
 ...
 *   funcionesPuesto      (texto del formulario)
 *   descansoDiario       (texto del formulario)
 *   fechaInicioOverride  "YYYY-MM-DD" (opcional)
 *   fechaFinOverride     "YYYY-MM-DD" (opcional)
 */
function generarContrato(payload) {
  var idInterno = (payload.idInterno || "").toString().trim();
  if (!idInterno) return { status: "error", message: "Falta ID INTERNO." };
  ...
  try {
    ...
    var idx = {
      ...
      puesto:      headers.indexOf("PUESTO"),
      sueldoMens:  headers.indexOf("SUELDO MENSUAL"),
      frecPago:    headers.indexOf("FRECUENCIA DE PAGO"),
      nacionalidad:headers.indexOf("NACIONALIDAD"),
      edad:        headers.indexOf("EDAD"),
      genero:      headers.indexOf("GÉNERO"),
      estadoCivil: headers.indexOf("ESTADO CIVIL"),
      curp:        headers.indexOf("CURP"),
      rfc:         headers.indexOf("RFC"),
      domicilio:   headers.indexOf("DOMICILIO COMPLETO (CALLE, NÚMERO, COLONIA, CP, ESTADO Y MUNICIPIO)"),
      tipoIngreso: headers.indexOf("TIPO DE INGRESO"),
      tipoContrato:headers.indexOf("TIPO DE CONTRATO"),
      fechaInicio: headers.indexOf("FECHA DE INICIO DEL PRIMER CONTRATO"),
      fechaFinOperativo: headers.indexOf("FECHA DE VENCIMIENTO DEL TERCER CONTRATO"),
      fechaFinAdmin:     headers.indexOf("FECHA DE VENCIMIENTO DEL SEXTO CONTRATO")
    };
    if (idx.idInterno === -1) idx.idInterno = 50;

    // ─── Validación de columnas críticas: fallar claro, no silencioso ───
    var faltantes = [];
    Object.keys(idx).forEach(function(k){ if (idx[k] === -1) faltantes.push(k); });
    if (faltantes.length) {
      return { status: "error", message: "No se encontraron estas columnas en BASE DE DATOS: " + faltantes.join(", ") + ". Revisa los encabezados del Sheet." };
    }

    // Columna de carpeta del expediente: "CARPETA" es el nombre real en tu Sheet (AX).
    // Se deja "LINK EXPEDIENTE" como fallback por si alguna vez cambia (nombre usado
    // en subirContratoFirmado), y el índice fijo AX como último recurso.
    var colCarpeta = headers.indexOf("CARPETA");
    if (colCarpeta === -1) colCarpeta = headers.indexOf("LINK EXPEDIENTE");
    if (colCarpeta === -1) colCarpeta = 49; // AX, fallback fijo

    var fila = -1;
    for (var i = 1; i < data.length; i++) {
      if ((data[i][idx.idInterno] || "").toString().trim() === idInterno) { fila = i; break; }
    }
    if (fila === -1) return { status: "error", message: "Empleado no encontrado con ID " + idInterno };

    var filaSheet = fila + 1;
    var row = data[fila];

    var nombreTrabajador = (row[idx.nombre]      || "").toString().trim();
    var empresaNombre    = (row[idx.empresa]     || "").toString().trim();
    var puesto            = (row[idx.puesto]      || "").toString().trim();
    var sueldoMensual     = _limpiarNumero(row[idx.sueldoMens]);
    var frecuenciaPago    = (row[idx.frecPago]   || "").toString().trim();
    var nacionalidad      = (row[idx.nacionalidad] || "").toString().trim();
    var edad              = (row[idx.edad]       || "").toString().trim();
    var genero            = (row[idx.genero]     || "").toString().trim();
    var estadoCivil       = (row[idx.estadoCivil]|| "").toString().trim();
    var curp              = (row[idx.curp]       || "").toString().trim();
    var rfcTrabajador     = (row[idx.rfc]        || "").toString().trim();
    var domicilio         = (row[idx.domicilio]  || "").toString().trim();
    var tipoIngreso       = (row[idx.tipoIngreso]  || "").toString().trim();
    var tipoContrato      = (row[idx.tipoContrato] || "").toString().trim();
    var carpetaURL        = (row[colCarpeta]     || "").toString().trim();

    if (!/^operativ/i.test(tipoIngreso) && !/^administrativ/i.test(tipoIngreso)) {
      return { status: "error", message: "TIPO DE INGRESO debe ser 'Operativo' o 'Administrativo'. Valor actual: '" + tipoIngreso + "'." };
    }
    var esOperativo = /^operativ/i.test(tipoIngreso);

    if (sueldoMensual <= 0) {
      return { status: "error", message: "El sueldo mensual no es válido (" + row[idx.sueldoMens] + "). Verifica la columna SUELDO MENSUAL." };
    }

    // ─── Fechas: override del formulario, o las del Sheet ───
    var fechaInicioTxt, fechaFinTxt;
    var fechaInicioOverride = (payload.fechaInicioOverride || "").toString().trim();
    var fechaFinOverride     = (payload.fechaFinOverride     || "").toString().trim();

    if (fechaInicioOverride) {
      fechaInicioTxt = formatearFechaLarga(fechaInicioOverride);
      sheet.getRange(filaSheet, idx.fechaInicio + 1).setValue(_isoASerialSheets(fechaInicioOverride));
    } else {
      fechaInicioTxt = formatearFechaLarga(row[idx.fechaInicio]);
    }

    if (fechaFinOverride) {
      fechaFinTxt = formatearFechaLarga(fechaFinOverride);
      var colFin = esOperativo ? idx.fechaFinOperativo : idx.fechaFinAdmin;
      sheet.getRange(filaSheet, colFin + 1).setValue(_isoASerialSheets(fechaFinOverride));
    } else {
      fechaFinTxt = formatearFechaLarga(esOperativo ? row[idx.fechaFinOperativo] : row[idx.fechaFinAdmin]);
    }

    if (!fechaInicioTxt || !fechaFinTxt) {
      return { status: "error", message: "Faltan fechas de inicio/fin de contrato. Captúralas en el formulario o en el Sheet." };
    }

    var funcionesPuesto = (payload.funcionesPuesto || "").toString().trim();
    var descansoDiario   = (payload.descansoDiario   || "30 minutos").toString().trim();
    if (!funcionesPuesto) return { status: "error", message: "Debes capturar las funciones del puesto en el formulario." };

    // ─── Datos de EMPRESAS ───
    var hojaEmp = ss.getSheetByName("EMPRESAS");
    var datosEmp = hojaEmp.getDataRange().getValues();
    var headersEmp = datosEmp[0].map(function(h){ return h ? h.toString().trim() : ""; });
    var eIdx = {
      empresa: headersEmp.indexOf("EMPRESA"),
      rfc:     headersEmp.indexOf("RFC"),
      domicilioFiscal: headersEmp.indexOf("DOMICILIO FISCAL"),
      actividad: headersEmp.indexOf("ACTIVIDAD ECONÓMICA PRINCIPAL"),
      repLegal: headersEmp.indexOf("REPRESENTANTE LEGAL")
    };
    var faltantesEmp = [];
    Object.keys(eIdx).forEach(function(k){ if (eIdx[k] === -1) faltantesEmp.push(k); });
    if (faltantesEmp.length) {
      return { status: "error", message: "No se encontraron estas columnas en EMPRESAS: " + faltantesEmp.join(", ") + "." };
    }

    var filaEmp = -1;
    for (var j = 1; j < datosEmp.length; j++) {
      if ((datosEmp[j][eIdx.empresa] || "").toString().trim() === empresaNombre) { filaEmp = j; break; }
    }
    if (filaEmp === -1) return { status: "error", message: "No se encontró la empresa '" + empresaNombre + "' en el catálogo EMPRESAS." };

    var rowEmp = datosEmp[filaEmp];
    var razonSocial   = (rowEmp[eIdx.empresa]        || "").toString().trim();
    var rfcEmpresa    = (rowEmp[eIdx.rfc]            || "").toString().trim();
    var domicilioFiscal = (rowEmp[eIdx.domicilioFiscal] || "").toString().trim();
    var objetoSocial  = (rowEmp[eIdx.actividad]      || "").toString().trim();
    var apoderadoLegal= (rowEmp[eIdx.repLegal]       || "").toString().trim();

    // ─── Salario diario ───
    var salarioDiarioNum = sueldoMensual / 30;
    var salarioDiarioTxt = "$" + salarioDiarioNum.toFixed(2) + " (" + numeroALetrasMoneda(salarioDiarioNum) + " M.N.)";

    var placeholders = {
      "{{RAZON_SOCIAL}}":            razonSocial,
      "{{APODERADO_LEGAL}}":         apoderadoLegal,
      "{{NOMBRE_COMPLETO}}":         nombreTrabajador,
      "{{RFC_EMPRESA}}":             rfcEmpresa,
      "{{DOMICILIO_FISCAL_EMPRESA}}":domicilioFiscal,
      "{{NACIONALIDAD}}":            nacionalidad,
      "{{EDAD}}":                    edad,
      "{{SEXO}}":                    genero,
      "{{ESTADO_CIVIL}}":            estadoCivil,
      "{{DOMICILIO_TRABAJADOR}}":    domicilio,
      "{{CURP}}":                    curp,
      "{{RFC_TRABAJADOR}}":          rfcTrabajador,
      "{{OBJETO_SOCIAL}}":           objetoSocial,
      "{{PUESTO}}":                  puesto,
      "{{FECHA_INICIO}}":            fechaInicioTxt,
      "{{FECHA_FIN_PERIODO}}":       fechaFinTxt,
      "{{TIPO_DE_CONTRATO}}":        tipoContrato,
      "{{FUNCIONES_PUESTO}}":        funcionesPuesto,
      "{{DESCANSO_DIARIO}}":         descansoDiario,
      "{{SALARIO_DIARIO_NUM}}":      salarioDiarioTxt,
      "{{PERIODICIDAD_PAGO}}":       frecuenciaPago
    };

    var idPlantilla = esOperativo ? ID_PLANTILLA_CONTRATO_OPERATIVO : ID_PLANTILLA_CONTRATO_ADMINISTRATIVO;

    var carpetaDestino;
    var matchCarpeta = carpetaURL.match(/\/folders\/([a-zA-Z0-9_-]+)/);
    if (matchCarpeta) {
      try { carpetaDestino = DriveApp.getFolderById(matchCarpeta[1]); } catch(e) { carpetaDestino = null; }
    }
    if (!carpetaDestino) {
      return { status: "error", message: "No se encontró la carpeta de expediente del empleado. Verifica la columna CARPETA (AX)." };
    }

    var nombreArchivo = "Contrato_" + idInterno + "_" + nombreTrabajador.replace(/[^a-zA-Z0-9 ]/g,"").trim();
    var copia = DriveApp.getFileById(idPlantilla).makeCopy(nombreArchivo, carpetaDestino);
    var doc  = DocumentApp.openById(copia.getId());
    var body = doc.getBody();

    Object.keys(placeholders).forEach(function(ph) {
      var patron      = ph.replace(/[{}]/g, "\\$&");             // escapa { } en el patrón de búsqueda
      var reemplazo   = _escaparReemplazo(placeholders[ph]);     // escapa $ y \ en el texto de reemplazo
      body.replaceText(patron, reemplazo);
    });
    doc.saveAndClose();

    var pdfBlob = copia.getAs("application/pdf");
    pdfBlob.setName(nombreArchivo + ".pdf");
    var pdfArchivo = carpetaDestino.createFile(pdfBlob);

    return {
      status: "success",
      message: "Contrato generado correctamente.",
      urlDoc: copia.getUrl(),
      urlPdf: pdfArchivo.getUrl(),
      tipoIngreso: esOperativo ? "Operativo" : "Administrativo"
    };

  } catch (ex) {
    Logger.log("[generarContrato] Error: " + ex);
    return { status: "error", message: "Error al generar contrato: " + ex.toString() };
  }
}

// ─── Escapa $ y \ para que DocumentApp.replaceText no los trate
//     como referencias de grupo de captura (bug real de GAS) ───
function _escaparReemplazo(texto) {
  return (texto || "").toString().replace(/\\/g, "\\\\").replace(/\$/g, "$$$$");
}

// Limpia strings tipo "$8,000.00" antes de convertir a número
function _limpiarNumero(valor) {
...
function _isoASerialSheets(isoStr) {
  var partes = isoStr.split("-");
  var fecha = new Date(Date.UTC(parseInt(partes[0]), parseInt(partes[1]) - 1, parseInt(partes[2])));
  var epocaSheets = new Date(Date.UTC(1899, 11, 30));
  return Math.round((fecha.getTime() - epocaSheets.getTime()) / 86400000);
}

// ─── HELPER: Fecha (serial, Date o "YYYY-MM-DD") → texto largo ──
  ...
  if (valor instanceof Date) {
    ...
    if (partes.length === 3) {
      if (partes[0].length === 4) {
        fecha = new Date(Date.UTC(parseInt(partes[0]), parseInt(partes[1]) - 1, parseInt(partes[2])));
      } else {
        fecha = new Date(Date.UTC(parseInt(partes[2]), parseInt(partes[1]) - 1, parseInt(partes[0])));
      }
    } else {
      return "";
    }
...
// ─── HELPER: Número a letras (pesos con centavos) ──────────────
function numeroALetrasMoneda(numero) {
  var entero   = Math.floor(numero);
  var centavos = Math.round((numero - entero) * 100);
  var letrasEntero = _numeroALetras(entero);
  var centavosTxt = (centavos < 10 ? "0" : "") + centavos;
  return _capitalizar(letrasEntero) + " pesos " + centavosTxt + "/100";
}
...
function _numeroALetras(n) {
  ...
  var ESPECIALES = { 11:"once",12:"doce",13:"trece",14:"catorce",15:"quince",16:"dieciséis",17:"diecisiete",18:"dieciocho",19:"diecinueve",
  ...
  function convertirGrupo(num) {
    ...
    if (resto > 0) {
      if (ESPECIALES[resto]) {
        ...
        var u = resto % 10;
        if (d === 1 && u > 0) {
          partes.push(ESPECIALES[10 + u]);
        } else {
          var textoDecena = DECENAS[d];
          if (u > 0) textoDecena += " y " + UNIDADES[u];
          partes.push(textoDecena);
        }
      }
    }
    return partes.join(" ");
  ...
  if (n < 1000000) {
    ...
    var resto = n % 1000;
    var textoMiles = (miles === 1) ? "mil" : convertirGrupo(miles) + " mil";
    return resto > 0 ? textoMiles + " " + convertirGrupo(resto) : textoMiles;
  }

  var millones = Math.floor(n / 1000000);
  var restoMillones = n % 1000000;
  var textoMillones = (millones === 1) ? "un millón" : convertirGrupo(millones) + " millones";
  var textoResto = restoMillones > 0 ? " " + _numeroALetras(restoMillones) : "";
